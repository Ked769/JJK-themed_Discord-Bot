...
async def fight2(ctx, char, bot, key, genome, net):
    ...
    embed = discord.Embed(title=f"{member1.name} vs Curse", colour=discord.Colour.blue())
...
class RPS(discord.ui.View):
    ...
    async def no(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.author.id != interaction.user.id:
            ...
            # noinspection PyUnresolvedReferences
            await interaction.response.send_message("You have turned down the quest", ephemeral=True)
            await interaction.message.delete()
